...
var engine = {
  ...
  init: function() { // Initialization
    ...
    addShadowedLight( -1, 1, 0, 0xffffff, .5 );
    addShadowedLight( 1, 1, 0, 0xffffff, .5 );
  ...
  loadModels: function() {
   ...
   oStlLoader.load('stl/engine.stl', function(geometry) {
     
     material = new THREE.MeshPhongMaterial({
      color:0x000000,specular:0x111111,shininess:200 });
     mesh = new THREE.Mesh( geometry, material );

    mesh.position.set( -0.33 , -0.15, 0  );
...
if (window.addEventListener)
  ...
  window.attachEvent('onload', finalizeEngine);
else window.onload = finalizeEngine;
